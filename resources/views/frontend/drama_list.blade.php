        ...
        <div class="filter">
            ...
            <select id="select-year">
                <option value="">-Select Year-</option>
                 @foreach(realse_year_menu() as $year)
        ...
        <div>
            ...
            <div class="list-content">
                <div class="title-list">
                    <a href="{{url('drama-list/char-start-'.strtolower($key))}}">View More</a>
                    <h4>{{$key}}</h4>
                </div>
            ...
            @foreach($row as $list)
                <li class="show filter-item year_{{$list['year']}} country_{{$list['country']}} status_{{$list['status']}}"
                 ...
                 <a href="{{url('drama-detail/'.$list['slug'])}}">{{$list['title']}}</a>
                </li>
            @endforeach
